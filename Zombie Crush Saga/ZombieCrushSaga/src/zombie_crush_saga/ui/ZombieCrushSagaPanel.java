...
//            renderJellyTiles(g);
            ...
            for (int j = 0; j < data.getGridRows(); j++)
            {
                for (int i = 0; i < data.getGridColumns(); i++)
                {
                    ...
                    if (newArray[i][j] == 1)
                    {
                        ...
                        g.fill3DRect(data.calculateTileXInGrid(i, 0), data.calculateTileYInGrid(j, 0), TILE_IMAGE_WIDTH, TILE_IMAGE_HEIGHT, true);
                    }
                }
            }
        }
    ...
    public void renderTileGridBackGround(Graphics g)
    {
        ...
        Image img = toolkit.getImage(imgPath+props.getProperty(ZombieCrushSaga.ZombieCrushSagaPropertyType.BLANK_TILE_IMAGE_NAME));
        if (miniGame.isCurrentScreenState(GAME_SCREEN_STATE))
        {
            int[][] newArray = new int[data.getGridColumns()][data.getGridRows()];
...
//                        g.drawImage(blankTileImage, i, i, this);
                        
                        g.drawImage(img, (int) data.calculateTileXInGrid(i, 0), (int) data.calculateTileYInGrid(j, 0),TILE_IMAGE_WIDTH,TILE_IMAGE_HEIGHT, null);
//                        g.fillOval(data.calculateTileXInGrid(i, 0), data.calculateTileYInGrid(j, 0), TILE_IMAGE_WIDTH, TILE_IMAGE_HEIGHT);
...
//                                , TILE_IMAGE_WIDTH, TILE_IMAGE_HEIGHT);
    ...
    {
        for(int i=0;i<data.getscoresToRenderSize();i++)
        {
            FloatingPointRender fp = data.getscoresToRender(i);
            renderPointsHelper(g,fp);
        }
    }
    public void renderPointsHelper(Graphics g, FloatingPointRender fp)
    {
        g.setColor(Color.BLACK);
        g.setFont(DEBUG_TEXT_FONT);
        g.drawString(""+fp.getScore(), (int)fp.getX(),(int)fp.getY());
    }
...
//            if (!data.won())
    ...
    public void randerScoreInGame(Graphics g)
    {
        if (((ZombieCrushSagaMiniGame) game).isCurrentScreenState(GAME_SCREEN_STATE))
        {
            if(!data.won())
            {
                ...
                g.drawString("Target: " + target, 0, 100);
                g.drawString("Moves: " + data1.getCounter(), 0, 200);
    ...
    //                FIRST THE ACTUAL BAR
                    ...
                    float barPercentage1;
                    
                    if(data.getScore() < data.getBronzeTarget())
                    {
                        barPercentage1 = ((float)data.getScore()/data.getBronzeTarget());
                    }
                    else if(data.getScore() < data.getGoldTarget())
                    {
                        barPercentage1 = ((float)data.getScore()/data.getGoldTarget());
                    }
                    else if(data.getScore() < data.getPlatinumTarget())
                    {
                        barPercentage1 = ((float)data.getScore()/data.getGoldTarget());
                    }
                    else
                    ...
                    float barWidth = barPercentage1 * (PROGRESS_BAR_CORNERS.right - PROGRESS_BAR_CORNERS.left);
